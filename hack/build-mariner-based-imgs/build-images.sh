...
print_usage() {
    ...
    echo ""
    echo "Script usage: ./build-images.sh -H <hypervisor (ch/qemu)> [-R <additional-rpms-dir>]"
...
while getopts ":hH:R:" option; do
    ...
    R) # Directory containing additional RPMs
        echo "Setting additional RPM dir..."
...
# After setting hypervisor, if it's 'ch' and additionalRpmsDir is empty, set default
if [[ $vmm == "ch" && -z "$additionalRpmsDir" ]]; then
    additionalRpmsDir=./additionalRpms/
    mkdir -p $additionalRpmsDir
fi

if [ -n "$additionalRpmsDir" ]; then
...
KUBEVIRT_COMPONENTS_TO_BUILD="${KUBEVIRT_COMPONENTS:-virt-operator virt-api virt-handler virt-controller virt-launcher virt-exportproxy}"
...
for ctr in virt-operator virt-api virt-handler virt-controller virt-exportproxy; do
