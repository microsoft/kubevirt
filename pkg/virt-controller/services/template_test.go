	...
	kv := &v1.KubeVirt{
		...
		Spec: v1.KubeVirtSpec{
			Configuration: v1.KubeVirtConfiguration{
				...
				VirtualizationStack: &QemuVirtualizationStackSpec,
			},
		},
...
`,
		...
		Context("with sriov interface", func() {
			...
			It("should add 1G of memory overhead", func() {
				...
				expectedMemory.Add(GetMemoryOverhead(vmi, arch, config.GetConfig().AdditionalGuestMemoryOverheadRatio, kv.Spec.Configuration.VirtualizationStack))
			...
			It("should still add memory overhead for 1 core if cpu topology wasn't provided", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				expectedMemory := resource.NewScaledQuantity(0, resource.Kilo)
				expectedMemory.Add(GetMemoryOverhead(vmi1, arch, config.GetConfig().AdditionalGuestMemoryOverheadRatio, kv.Spec.Configuration.VirtualizationStack))
				expectedMemory.Add(*vmi.Spec.Domain.Resources.Requests.Memory())
				Expect(pod.Spec.Containers[0].Resources.Requests.Memory().Value()).To(Equal(expectedMemory.Value()))
		...
		Context("with guest-to-request memory headroom", func() {
			...
			DescribeTable("should add guest-to-memory headroom when configured with ratio", func(ratioStr string) {
				...
				originalOverhead := GetMemoryOverhead(vmi, config.GetClusterCPUArch(), nil, kv.Spec.Configuration.VirtualizationStack)
				actualOverheadWithHeadroom := GetMemoryOverhead(vmi, config.GetClusterCPUArch(), pointer.P(ratioStr), kv.Spec.Configuration.VirtualizationStack)
		...
		When("using auto resource limits ", func() {
			Context("when the creation namespace has a resource quota with memory limits associated to it", func() {
				...
				DescribeTable("should not override limits", func(withLimits, withDedicatedCPU bool) {
					...
					Expect(pod.Spec.Containers[0].Name).To(Equal("compute"))
					expectedMemory := resource.NewScaledQuantity(0, resource.Kilo)
					expectedMemory.Add(GetMemoryOverhead(&vmi, defaultArch, config.GetConfig().AdditionalGuestMemoryOverheadRatio, kv.Spec.Configuration.VirtualizationStack))
